...
def test_deactivate_company(
    ...
    assert companies_page.get_company_status(
        data["company_name"]
    ) == "Active"

    # Deactivate Company

    companies_page.click_deactivate_company(
        data["company_name"]
    )

    companies_page.click_deactivate_button()

    assert companies_page.is_company_deactivated_successfully()
...
@pytest.mark.tc_comp_031
def test_edit_action_disabled_for_deactivated_company(
        companies_page
):
    ...
    assert companies_page.is_edit_button_disabled(
        data["company_name"]
    ), "Edit button should be disabled"

    # Verify Tooltip Text

    assert companies_page.get_edit_button_tooltip(
        data["company_name"]
    ) == "Unavailable while the company is deactivated."
